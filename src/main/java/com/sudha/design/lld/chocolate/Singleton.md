...
`ensures a class only has one instance of it and provides a global point of access to it.`
...
`We finally come up with a tradeoff in DoubleCheckedLockingSingleton wherein the getInstance method is not synchronised but only the instantiation block
is in a synchronised block removing the slowness of synchronised method for further all calls.`
